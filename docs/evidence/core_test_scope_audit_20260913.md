...
当前分支 `codex/feature-session-engine-integration` 在本地通过 `296 passed`、`compileall` 和 `git diff --check`。这些测试主要是纯函数、注入式 Provider、冻结 fixture 和确定性 replay；它们不是每次运行都连接生产数据。
...
| 层级 | 范围 | 证据 |
| --- | --- | --- |
| Unit | Contract、Clock/Calendar、Q2、Window、Data、Facts、Engine、Replay | 本地 pytest 296 passed |
