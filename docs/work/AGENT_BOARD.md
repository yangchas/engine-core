...
|---|---|---|---|---|---|
| TASK-008 Real opening validation (read-only) | replay-investigator | PARTIAL_EVIDENCE | `codex/feature-session-engine-integration` | current development worktree | audit HOLD; real Q2 evidence and Engine FACT_ONLY shadow recorded; NORMAL 09:32:10 evidence still pending; no production source or write path |
...
soft-cutoff timing gaps, then closed after the fixes, real-data evidence, and
independent auditor PASS. No overlapping write is allowed.
