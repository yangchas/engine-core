...
- [VERIFIED] 2026-09-18 commit `4ca5915` adds `PreviousDayLimitStructureFact` as a pure, fact-only projection of the guarded previous-session limit-up pool. A real Cobra-ion Redis capture had 47 rows, HLEN/scan consistency, and zero decode errors; because historical `available_at_ms` is absent, runtime fact output remains `UNAVAILABLE` and does not leak rows. Local and Cobra-ion suites both pass `486`; compileall passes; real fixture SHA is `00c6376d1ca8587c15be7aadb5ff3f08d7c76f5cc45db0d782d1788daa290c89`. This closes only previous-session structure, not current return feedback, plate/locked-order parity, or full report ownership. Evidence: `docs/evidence/previous_day_limit_structure_fact_20260918.md`.
