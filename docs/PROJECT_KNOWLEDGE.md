...
- [VERIFIED] 当前只读生产链审计工具代码验证 commit 为 `e0853010de31e5acc891be61d497073da377f1d7`；本地与 cobra-ion Python 3.12.3 均执行 `319 passed`、`compileall` 通过，真实 capture/TD 样本生成的六类审计产物字节级 SHA-256 一致。当前摘要已将 `AuctionState` 降为 `OBSERVED`、存储投影降为 `WARN`，并将 `engine_core_q2_path=PASS` 与完整竞价 Shadow `PARTIAL` 分开。此前的 `9392f1e`、`e106e12`、`0b6021c` 及更早测试/commit 记录均为历史证据，不代表当前代码身份；详见 `docs/production_chain_shadow_20260914.md`。
