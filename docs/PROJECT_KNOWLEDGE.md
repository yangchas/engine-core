...
- [VERIFIED] 2026-09-18 16:38 CST the capability-local legacy report status parity test now covers every Core `FactStatus`: `READY→COMPLETE`, `PARTIAL→PARTIAL`, and `MISSING/INVALID/UNAVAILABLE→DATA_UNAVAILABLE`. This closes only the deterministic report-status boundary; full legacy email/plate/locked-order/delivery/strategy parity remains open. No production source, storage, Rabbit, or effect path changed. Evidence: `docs/evidence/legacy_report_status_parity_20260918.md`.
